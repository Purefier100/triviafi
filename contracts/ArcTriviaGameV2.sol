...
// =============================================================================
// ArcTriviaGame — Final Version
// Arc Network (Chain ID: 5042002) | USDC native gas
//
// Features:
//  - Creator sets registration deadline (join window) + play deadline
//  - Game waits for registration deadline before anyone can play
//  - Top 3 prize distribution based on player count
//  - Anyone can create games, creator earns 2.5%
//  - Platform earns 2.5%
//  - Full leaderboard, player stats, prize breakdown
//  - Auto-end when all players done or deadline passed
// =============================================================================
...
contract ArcTriviaGame {
    // ── Constants ──
    uint256 public constant MIN_ENTRY_FEE = 1e8; // 1 zkLTC (8 decimals)
    uint256 public constant PLATFORM_BPS = 250; // 2.5%
    ...
    struct Game {
        ...
        uint256 maxPlayers;
        uint256 prizePool;
        uint256 registrationEnd; // players can join until this time
        uint256 playDeadline; // players must finish by this time
    ...
    event RoomFull(uint256 indexed id);
    ...
    constructor(address _verifier, address _usdc) {
        platform = msg.sender;
        verifier = _verifier;
        usdc = IERC20(_usdc);
    }
    ...
    function createGame(
        ...
        require(bytes(categoryName).length > 0, "Category empty");
        require(entryFee >= MIN_ENTRY_FEE, "Min entry");
        require(maxPlayers >= 1 && maxPlayers <= 100, "1-100 players");
        require(registrationSecs >= 60, "Min 60s registration");
        require(
            playDeadlineSecs >= 60 && playDeadlineSecs <= 86400,
            "60s-24h play window"
        );
    ...
    // =========================================================================
    // JOIN — only during registration window
    // =========================================================================
    function joinGame(uint256 id) external payable gameExists(id) {
        ...
        require(
            usdc.transferFrom(msg.sender, address(this), g.entryFee),
    ...
    // =========================================================================
    // SUBMIT SCORE — only after registration ends, before play deadline
    // =========================================================================
    function submitScore(
        uint256 id,
        uint256 score,
        bytes memory signature
    ) external gameExists(id) {
        Game storage g = games[id];
        ...
        require(
            block.timestamp >= g.registrationEnd,
        ...
        bytes32 message = keccak256(
            abi.encodePacked(msg.sender, id, score, nonce)
        );

        require(
            recoverSigner(message, signature) == verifier,
            "Invalid signature"
        );
        ...
        if (g.finishedCount >= g.players.length) {
            _endGame(id);
        }
    }
    // =========================================================================
    // TRIGGER END — callable by anyone after play deadline
    // =========================================================================
    function triggerEnd(uint256 id) external gameExists(id) {
        Game storage g = games[id];
        require(g.status == GameStatus.Open, "Already ended");
        require(
            block.timestamp > g.playDeadline ||
                g.finishedCount >= g.players.length,
            "Not ready to end"
        );
        _endGame(id);
    }
    ...
    // =========================================================================
    // CLAIM PRIZE — top 3 each claim separately
    // =========================================================================
    function claimPrize(uint256 id) external gameExists(id) {
        Game storage g = games[id];
        require(g.status == GameStatus.Ended, "Game not ended");
        require(!hasClaimed[id][msg.sender], "Already claimed");

        // Find caller's position (1, 2, or 3)
        uint8 pos = 0;
        ...
        require(pos > 0, "Not a prize winner");
    ...
    function cancelGame(
        ...
        Game storage g = games[id];
        require(
            msg.sender == g.creator || msg.sender == platform,
            "Not authorized"
        );
    ...
    function claimRefund(uint256 id) external gameExists(id) {
        ...
        g.prizePool -= g.entryFee;
        require(usdc.transfer(msg.sender, g.entryFee), "Refund failed");
    ...
    function _endGame(uint256 id) internal {
        ...
        if (n == 0) {
            g.status = GameStatus.Cancelled;
            emit GameCancelled(id, "No players");
            return;
        }
        ...
        for (uint256 i = 0; i < n; i++) {
            ...
            if (top[0] == address(0) || score > scores[id][top[0]]) {
                top[2] = top[1];
                top[1] = top[0];
                top[0] = player;
            } else if (top[1] == address(0) || score > scores[id][top[1]]) {
                top[2] = top[1];
                top[1] = player;
            } else if (top[2] == address(0) || score > scores[id][top[2]]) {
                top[2] = player;
            }
        }
        ...
        if (creatorFee > 0 && !hasJoined[id][g.creator]) {
            require(usdc.transfer(g.creator, creatorFee), "Creator fee failed");
        }
    ...
    function _calcPrize(
        ...
        // 95% distributable (after 5% fees)
        uint256 dist = (pool * 9500) / 10000;
        if (n == 1) {
            return pos == 1 ? dist : 0;
        } else if (n == 2) {
            if (pos == 1) return (dist * 7000) / 10000; // 70%
            if (pos == 2) return (dist * 3000) / 10000; // 30%
        } else {
            if (pos == 1) return (dist * 6000) / 10000; // 60%
            if (pos == 2) return (dist * 2500) / 10000; // 25%
            if (pos == 3) return (dist * 1500) / 10000; // 15%
        }
        return 0;
    }
    ...
    function getGame(
        uint256 id
    ) external view gameExists(id) returns (GameView memory) {
        Game storage g = games[id];
        ...
        return
            GameView({
                ...
                registrationEnd: g.registrationEnd,
                playDeadline: g.playDeadline,
                topPlayers: g.topPlayers,
    ...
    function getPlayers(
        uint256 id
    ) external view gameExists(id) returns (address[] memory) {
    ...
    function getPlayerStatus(
        ...
        external
        view
        returns (bool joined_, bool finished_, bool claimed_, uint256 score_)
    {
        return (
            hasJoined[id][player],
            hasFinished[id][player],
            hasClaimed[id][player],
            scores[id][player]
        );
    }
    ...
    function getPlayerStats(
        address player
    ) external view returns (uint256 played, uint256 won, uint256 earned) {
    ...
    function getLeaderboard(
        ...
        returns (
            ...
            bool[] memory claimed_
        )
    {
        Game storage g = games[id];
        uint256 n = g.players.length;
        addrs = new address[](n);
        scoreList = new uint256[](n);
        finished = new bool[](n);
        claimed_ = new bool[](n);
        for (uint256 i = 0; i < n; i++) {
            address p = g.players[i];
            addrs[i] = p;
            scoreList[i] = scores[id][p];
            finished[i] = hasFinished[id][p];
            claimed_[i] = hasClaimed[id][p];
        }
    }
        ...
        )
    {
        Game storage g = games[id];
        uint256 n = g.players.length;
        platformFee_ = (g.prizePool * PLATFORM_BPS) / 10000;
        first = _calcPrize(g.prizePool, n, 1);
        second = _calcPrize(g.prizePool, n, 2);
        third = _calcPrize(g.prizePool, n, 3);
    }
