...
async function notifyBot(payload) {
  ...
  try {
    ...
    log(`⚠️ Bot webhook failed: ${e.message}`);
  }
}
...
async function makeArcProvider() {
  ...
  return new ethers.JsonRpcProvider(ARC_RPCS[0], {
    chainId: ARC_CHAIN_ID,
    name: "arc-testnet",
  });
}
...
function makeLitvmProvider() {
  return new ethers.JsonRpcProvider(LITVM_RPC, {
    chainId: 4441,
    name: "litvm",
  });
}
...
async function scanChain(contract, agentAddress, chainName) {
  ...
  log(
    `📋 [${chainName}] Agent open: ${agentOpen.length} | Endable: ${endableIds.length}`,
  );
...
async function createLitvmGame(litvmContract, litvmProvider, agentAddress) {
  ...
  if (bal < gasBuffer) {
    log(
      `❌ [LitVM] Insufficient zkLTC for gas: have ${ethers.formatEther(bal)}`,
    );
    return false;
  }
  ...
  while (retries > 0) {
    try {
      ...
      // ── Notify Discord bot ────────────────────────────────────────────
      const newGameId = Number(await litvmContract.gameCounter());
      notifyBot({
        type: "agent_room_created",
        gameId: newGameId,
        gameName: room.name,
        entryFee: LITVM_ENTRY_FEE,
        tokenSymbol: "zkLTC",
        maxPlayers: MAX_PLAYERS,
        chainId: 4441,
      });
...
async function tick(
  ...
  const [arcState, litvmState] = await Promise.all([
    scanChain(arcContract, arcAddress, "Arc").catch((e) => {
      log(`❌ Arc scan failed: ${e.message}`);
      return { agentOpen: [], endableIds: [], count: 0 };
    }),
    scanChain(litvmContract, litvmAddress, "LitVM").catch((e) => {
      log(`❌ LitVM scan failed: ${e.message}`);
      return { agentOpen: [], endableIds: [], count: 0 };
    }),
  ]);
  ...
  // Arc cycle
  const arcActive = arcState.agentOpen.filter(
    (g) => !arcState.endableIds.includes(g.id),
  );
  const arcCooldownOk =
    arcLastCycle === 0 || (now - arcLastCycle) / 3600 >= CYCLE_COOLDOWN_HRS;
  log(
    `⏱️  [Arc]   Hours since last cycle: ${arcLastCycle === 0 ? "never" : ((now - arcLastCycle) / 3600).toFixed(1)}`,
  );
  ...
  if (arcActive.length === 0 && arcCooldownOk) {
    ...
    if (ok) {
      arcLastCycle = now;
      log(`✅ [Arc] Cycle complete — next in ${CYCLE_COOLDOWN_HRS}hrs`);
    }
  } else if (arcActive.length > 0) {
    log(`⏳ [Arc] ${arcActive.length} active game(s) — waiting`);
  } else {
    log(
      `⏳ [Arc] Cooldown — ${(CYCLE_COOLDOWN_HRS - (now - arcLastCycle) / 3600).toFixed(1)}hrs left`,
    );
  }
  ...
  // LitVM cycle
  const litvmActive = litvmState.agentOpen.filter(
    (g) => !litvmState.endableIds.includes(g.id),
  );
  const litvmCooldownOk =
    litvmLastCycle === 0 || (now - litvmLastCycle) / 3600 >= CYCLE_COOLDOWN_HRS;
  log(
    `⏱️  [LitVM] Hours since last cycle: ${litvmLastCycle === 0 ? "never" : ((now - litvmLastCycle) / 3600).toFixed(1)}`,
  );

  if (litvmActive.length === 0 && litvmCooldownOk) {
    const ok = await createLitvmGame(
      litvmContract,
      litvmProvider,
      litvmAddress,
    );
    if (ok) {
      litvmLastCycle = now;
      log(`✅ [LitVM] Cycle complete — next in ${CYCLE_COOLDOWN_HRS}hrs`);
    }
  } else if (litvmActive.length > 0) {
    log(`⏳ [LitVM] ${litvmActive.length} active game(s) — waiting`);
  } else {
    log(
      `⏳ [LitVM] Cooldown — ${(CYCLE_COOLDOWN_HRS - (now - litvmLastCycle) / 3600).toFixed(1)}hrs left`,
    );
  }
}
...
async function run() {
  ...
  if (!process.env.LITVM_AGENT_KEY) {
    console.error("❌ Missing LITVM_AGENT_KEY");
    process.exit(1);
  }
  ...
  const litvmWallet = new ethers.Wallet(
    process.env.LITVM_AGENT_KEY,
    litvmProvider,
  );
  ...
  try {
    log(
      `Arc   balance: ${ethers.formatEther(await arcProvider.getBalance(arcWallet.address))} ARC`,
    );
  } catch (_) {}
  try {
    log(
      `LitVM balance: ${ethers.formatEther(await litvmProvider.getBalance(litvmWallet.address))} zkLTC`,
    );
  } catch (_) {}
